...
function StatPill({ label, value, tone = 'slate' }) {
  const tones = {
    cyan: 'border-white/30 bg-white/10 text-white',
    red: 'border-white/20 bg-white/5 text-white',
    green: 'border-white/30 bg-white/10 text-white',
  ...
  return (
    <div className={`border px-4 py-3 ${tones[tone]}`}>
      ...
      <p className="mt-1 text-lg font-bold font-serif">{value}</p>
    </div>
  );
}
...
export default function Disaster() {
  ...
  return (
    <div className="-m-8 min-h-screen bg-[#07111f] bg-[radial-gradient(circle_at_20%_10%,rgba(14,165,233,0.18),transparent_28%),linear-gradient(rgba(148,163,184,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.06)_1px,transparent_1px)] bg-[length:auto,36px_36px,36px_36px] p-6 text-slate-100">
      <div className="mb-5 flex flex-col gap-4  border border-white/10 bg-white/5 p-5 shadow-2xl backdrop-blur lg:flex-row lg:items-center lg:justify-between">
        <div>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            disabled={checking}
            className="flex items-center gap-2  bg-red-500 px-4 py-2 font-bold text-white shadow-lg shadow-red-950/40 transition hover:bg-red-400 disabled:opacity-50"
          >
            <PlayCircle size={18} />
            Simulate Disaster
          </button>
          <button
            ...
            disabled={checking}
            className="flex items-center gap-2  border border-cyan-300/30 bg-cyan-400/10 px-4 py-2 font-bold text-cyan-100 transition hover:bg-cyan-400/20 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <section className="relative h-[620px] overflow-hidden  border border-white/10 bg-slate-950 shadow-2xl">
          ...
          <div className="absolute bottom-5 left-5  border border-white/10 bg-slate-950/80 p-4 text-xs shadow-xl backdrop-blur">
            ...
            {[
              ...
              <div key={label} className="mb-2 flex items-center gap-2 text-slate-200 last:mb-0">
                <span className={`inline-block h-3 w-7  ${style}`} />
                {label}
              </div>
            ))}
          </div>
          ...
          {activeEvent && (
            <div className="absolute right-5 top-5 max-w-sm  border border-red-300/20 bg-red-950/70 p-4 shadow-xl backdrop-blur">
        ...
        <aside className="space-y-5">
          <section className=" border border-white/10 bg-white/7 p-5 shadow-2xl backdrop-blur">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="flex items-center gap-2 text-lg font-black">
                <Route className="text-cyan-300" />
                Route Decision
              </h2>
              {selectedRoute && <span className=" bg-emerald-400/15 px-3 py-1 text-xs font-bold text-emerald-200">Recommended</span>}
            </div>
            ...
            {selectedRoute ? (
              <div className="space-y-4">
                <select
                  ...
                  className="w-full  border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none"
                >
                  {routes.map((route) => (
                ...
                <div className=" border border-cyan-300/20 bg-cyan-400/10 p-4">
                ...
                <div className=" border border-white/10 bg-slate-950/60 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs">
                    <span className=" bg-white/10 px-3 py-1">Supplier reliability {Math.round((selectedRoute.supplier_reliability_score || 0) * 100)}%</span>
                    <span className=" bg-white/10 px-3 py-1">{selectedRoute.supplier_emergency_certified ? 'Emergency certified' : 'Standard supplier'}</span>
                    <span className=" bg-white/10 px-3 py-1">Mode {selectedRoute.alternate_mode || 'road'}</span>
                  </div>
                </div>
              </div>
            ) : (
          ...
          <section className={` border p-5 shadow-2xl backdrop-blur ${activeEvent ? 'border-red-300/20 bg-red-950/35' : 'border-emerald-300/20 bg-emerald-950/25'}`}>
          ...
          <section className=" border border-white/10 bg-white/7 p-5 shadow-2xl backdrop-blur">
            ...
            <div className="max-h-[180px] space-y-3 overflow-y-auto pr-1">
              {history.filter((event) => !event.is_active).slice(0, 5).map((event) => (
                <div key={event.event_id} className=" bg-white/5 p-3 text-sm">
      ...
      <section className="mt-5  border border-white/10 bg-white/7 p-5 shadow-2xl backdrop-blur">
        <div className="mb-4 flex flex-wrap gap-3">
          {[
            ...
            <button
              ...
              className={` px-4 py-2 text-sm font-bold transition ${activeTab === key ? 'bg-cyan-300 text-slate-950' : 'bg-white/5 text-slate-300 hover:bg-white/10'}`}
            >
              {label}
            </button>
          ))}
              ...
              <tbody>
                {routes.map((route) => (
                  <tr key={route.route_id} className="border-t border-white/10">
                    ...
                    <td className="px-3 py-3"><span className=" bg-cyan-300/15 px-3 py-1 text-xs font-bold text-cyan-200">Rerouted</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeTab === 'suppliers' && (
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {emergencySuppliers.map((supplier, index) => (
              <div key={supplier.supplier_id} className=" border border-white/10 bg-slate-950/50 p-4">
                ...
                <div className="mt-3 flex gap-2 text-xs">
                  <span className=" bg-white/10 px-3 py-1">{Math.round(supplier.reliability_score * 100)}% reliable</span>
                  <span className=" bg-white/10 px-3 py-1">{supplier.avg_lead_days}d lead</span>
                </div>
              </div>
            ))}
          </div>
